...
export default function MoodCheckScreen({navigation}) {
  ...
  const [moodsSelected, setMoods] = useState(initialState);

  const moodSelectChange = (index) => {    
    const mood = moodsSelected[index];
    ...
    console.log(
      moodsSelected[index].name + (
        moodsSelected[index].selected ? " selected." :  " unselected."
      ) )
  }
  ...
  const createMoodButtons = () => {
    ...
    return moodsSelected.map((mood, index) => (
